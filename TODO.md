...
## Phase 7 — 港股打新套利系统 IPO Stars

> **定位**：纯套利工具，非长期投资。核心是情绪 + 主题热度驱动的短期博弈。  \
> **运行模式**：Epidemic（批次驱动：招股 → 申购 → 暗盘 → 上市 → 结束），与现有 Continuous 日线策略完全解耦。  \
> **分析重心**：情绪因子 45% + 主题热度 25% + 发行条款 15% + 市场窗口 10%，基本面权重极低。

### P7-0：基础设施搭建

- [ ] **[P0] 新建分支 `feature/ipo-stars` + 初始化目录结构**
  - `core/ipo_data_source.py` — IPO 数据获取（东方财富/HKEX 接口）
  - `core/ipo_signals.py` — IPO 因子体系（情绪/主题/条款/窗口）
  - `core/ipo_analysis.py` — IPO 分析引擎（多因子 → 胜率/情景/建议）
  - `core/ipo_book.py` — 打新持仓账本（申购→暗盘→上市→卖出，独立于 PositionBook）
  - `core/ipo_risk.py` — 打新专项风控（破发止损/暗盘止盈/仓位限制）
  - `core/ipo_store.py` — 历史新股数据库（Parquet，data/ipo/）
  - `scheduler/ipo_scanner.py` — 每日 09:00 Cron Job 扫描港交所新股列表
  - `reports/ipo_renderer.py` — 飞书 Markdown 渲染器
  - `config/ipo_config.py` — 打新专项配置（因子权重/仓位限制/卖出策略）

- [ ] **[P0] EventBus 扩展，新增 IPO 事件类型**
  - `IPOFoundEvent` — 扫描到新招股
  - `IPOAnalysedEvent` — 分析完成
  - `IPOSellSignalEvent` — 卖出信号触发
  - 现有 `MarketEvent` / `SignalEvent` / `AlertEvent` 完全不变

- [ ] **[P0] AlertManager 复用验证**
  - IPO 报告通过 `AlertManager.send_critical/warning/info()` 推送
  - 频率限制、渠道配置完全复用现有逻辑

### P7-1：数据层 — `IPODataSource`

- [ ] **[P0] 东方财富 IPO 数据接口**
  - 招股中公司列表（发行价区间、募资额、申购时间、上市日期）
  - 聆讯资料（关键业务/财务数据）
  - 行业分类（机器人/AI/医疗/EV 等主题标签）

- [ ] **[P1] HKEX 披露易招股书解析**
  - 招股书关键数据提取（保荐人、承销团、股东结构）
  - 聆讯状态跟踪

- [ ] **[P1] 暗盘行情接口**
  - 暗盘交易时间段（上市前一交易日 16:15-18:30）抓取实时行情
  - 辉立/富途/耀才暗盘数据源

- [ ] **[P1] 市场情绪数据复用**
  - 接入现有 `CompositeMarketDataSource`（恒生科技/VIX/南向资金）
  - 零额外数据获取成本

### P7-2：因子体系 — `IPOSignals`

- [ ] **[P0] `IPOMarketSentimentFactor`（权重 45%，最高）**
  - 恒生科技指数当月表现（科技情绪代表）
  - VIX 恐慌指数（低于 15 = 风险偏好高）
  - 南向资金净流入
  - 直接来自 `CompositeMarketDataSource`

- [ ] **[P0] `IPOThemeHeatFactor`（权重 25%）**
  - 东方财富/同花顺主题板块指数（过去 7 天新闻密度）
  - 港股同主题近期上市新股平均涨幅（板块动量）
  - 新闻舆情密度（关键词：机器人/AI/医疗/EV/消费）
  - 行业分类标签

- [ ] **[P1] `IPOSubscriptionTermsFactor`（权重 15%）**
  - PS 相对行业均值折让率
  - 募资规模（5~15 亿最容易被炒作）
  - 发行定价区间宽度（窄 = 发行人信心足）
  - 保荐人历史胜率（海通/国君/中金数据库）

- [ ] **[P1] `IPOMarketWindowFactor`（权重 10%）**
  - 过去 30 天港股打新胜率（滚动窗口）
  - 恒生指数趋势判断（上升趋势中打新胜率高）
  - 美股隔夜情绪传导（纳斯达克 → 港股科技）

### P7-3：分析引擎 — `IPOAnalysisEngine`

- [ ] **[P0] 招股分析主流程**
  - 多因子加权评分 → 综合胜率（0~1）
  - 阈值：> 0.70 → AGGRESSIVE，0.55~0.70 → NEUTRAL，< 0.55 → SKIP

- [ ] **[P0] 情景蒙卡模拟**
  - 乐观 / 中性 / 保守三种情景 + 概率分布
  - 参考同类历史新股首日表现分位数（p75/median/p25）
...
- [ ] **[P1] 保荐人历史数据库**
  - 各保荐人历史新股数量、平均首日涨幅、胜率
  - 用于 `IPOSubscriptionTermsFactor` 的保荐人评分

- [ ] **[P1] 历史新股数据积累与回测**
  - 2023-2024 年港股新股数据回测胜率模型
  - 因子 IC 验证：情绪因子 IC vs 主题因子 IC

### P7-4：持仓与风控 — `IPOBook` + `IPORiskEngine`

- [ ] **[P0] `IPOPhase` 生命周期管理**
  - `SUBSCRIBED`（申购中，资金冻结）→ `DARK_POOL`（暗盘）→ `LISTED`（上市）→ `CLOSED`（已卖出）
  - 各阶段触发不同分析逻辑
...
- [ ] **[P1] 暗盘行情自动监控**
  - 暗盘交易日 16:15 自动拉取暗盘行情
  - 推送暗盘快报（含当前涨幅 + 建议操作）

- [ ] **[P1] 上市首日提醒**
  - 上市前一交易日 22:00 推送上市提醒
  - 上市当日 09:30 推送持仓状态 + 卖出确认

- [ ] **[P2] 持续跟踪与归因**
  - 每只新股卖出后写入 `IPORecordStore`
  - 季度输出打新收益归因报告（胜率/平均涨幅/最大单笔盈亏）

### P7-6：飞书推送 — `IPOReportRenderer`

- [ ] **[P0] 招股分析报告渲染**
  - Markdown 格式（适配飞书）
  - 包含：综合评分 + 六维雷达图文字版 + 情景预测 + 操作建议 + 风险提示

- [ ] **[P1] 暗盘快报渲染**
  - 实时涨幅 + 目标价 vs 当前价对比 + 操作按钮建议

- [ ] **[P1] 打新复盘报告**
  - 单只新股完整操作记录（申购价/暗盘价/上市价/最终收益）
  - 收益归因（情绪因子贡献 / 主题因子贡献 / 条款因子贡献）
...
| 文件 | 优先级 | 说明 |
|------|--------|------|
| `core/ipo_data_source.py` | P0 | IPO 数据获取（东方财富/HKEX/暗盘） |
| `core/ipo_signals.py` | P0 | IPO 因子体系（情绪/主题/条款/窗口） |
| `core/ipo_analysis.py` | P0 | 分析引擎（多因子 → 胜率/情景/建议） |
| `core/ipo_book.py` | P0 | 打新持仓账本（独立于 PositionBook） |
| `core/ipo_risk.py` | P0 | 打新专项风控（破发止损/暗盘止盈） |
| `core/ipo_store.py` | P1 | 历史新股数据库（Parquet） |
| `scheduler/ipo_scanner.py` | P0 | 每日 09:00 Cron Job |
| `reports/ipo_renderer.py` | P0 | 飞书 Markdown 渲染器 |
| `config/ipo_config.py` | P0 | 打新专项配置 |
| `core/event_bus.py`（扩展） | P0 | 新增 IPO 事件类型 |

### 与现有系统集成方式（零冲突）

| 现有组件 | 集成方式 | 说明 |
|---------|---------|------|
| `EventBus` | 扩展事件类型 | 复用，不复制，新增 3 个 IPO 事件 |
| `CompositeMarketDataSource` | 直接 import | 市场情绪数据 0 额外成本 |
| `AlertManager` | 直接复用 | IPO 渲染器 → `send_critical/warning/info()` |
| `SignalWeighter` | 直接复用 | IPO Signal 格式兼容 |
| `DataLayer` | Parquet 平行目录 | `data/ipo/` 与 `data/fundamental/` 平行 |
