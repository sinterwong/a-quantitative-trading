...
3. GET  /fundamentals/{symbol}        → PE/PB/ROE/股息率/市值
...
**GET /fundamentals/{symbol}**（注意路径无 `/basic`）
```json
{
  "pe": "18.01", "pb": 2.85, "dividend_yield": 0.16,
  "market_cap": 6498.76, "name": "长江电力", "price": 26.56,
  "symbol": "600900.SH", "status": "ok"
}
```
⚠️ `revenue_yoy` / `net_profit_yoy` / `industry` / `roe` 在此 API 中不存在（数据缺口）
...
def get_daily(symbol: str, days: int = 60) -> dict:
    return requests.get(f"{BASE}/data/daily/{symbol}", params={"days": days}, headers=HEADERS, timeout=10).json()
...
⚠️ **数据缺口**: `revenue_yoy` / `net_profit_yoy` / `industry` / `roe` 在 `GET /fundamentals/{symbol}` 中不存在，分析时需注明"YoY数据不可用"。
...
2. **YoY 数据缺失**: `GET /fundamentals/{symbol}` 不返回 `revenue_yoy`/`net_profit_yoy`，无法判断营收/净利润趋势
...
- [ ] 基本面字段完整展示（PE/PB/dividend_yield）
