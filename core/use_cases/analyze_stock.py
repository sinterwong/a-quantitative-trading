"""
core/use_cases/analyze_stock.py — 单股票综合分析 use case (P2-2)

> 历史路径:本文件原位于 ``backend/services/single_stock_analysis.py``。
> P2-2 重构起,业务下沉到 use case 层,backend wrapper 保留兼容性。
...
- ``analyze_stock(req)`` — **新增统一入口**,按 market 自动 dispatch
- ``analyze_a_share(req)`` / ``analyze_hk_share(req)`` — 保留(为兼容旧 caller)

每个分析步骤独立 try-except,单步失败不影响整体报告——
